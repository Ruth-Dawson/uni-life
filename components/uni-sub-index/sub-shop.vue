	...
	<view class="content" :style="{height:contentH+'px'}" >
			...
			<view class="listLeft">
				<view class="module" v-for="(item,index) in cardListLeft" :key="item.id" @click="showModule(item.index)">
					<view class="video">
						<video :src="item.url" show-center-play-btn="true" initial-time="5" controls="true"></video>
					</view>
					<view class="info">
						<view class="brief">
						...
						<view class="price-distance">
							...
							<view class="distance">
								<text class="distanceNum">{{item.distance}}人想要</text>
							</view>
						</view>
					</view>
				</view>
				...
				.module{
					...
					.video{
						...
						video{
							...
							// 视频高度应该不一致
							// 可以通过后台传过来的video分辨率动态设置高度，达到参差不齐的效果
							height: 250rpx;
							width: 350rpx;
						}
					}
